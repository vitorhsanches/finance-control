    ...
    <div className="page-stack">
      ...
      <section className="cards-grid">
        <MetricCard
          ...
          value={money(metrics.availableBalance, state)}
          tone={metrics.availableBalance >= 0 ? "good" : "bad"}
        />
        <MetricCard
        ...
        <MetricCard
          ...
          tone={metrics.monthExpenses > metrics.monthIncome ? "bad" : "neutral"}
        />
        <MetricCard
          label="Pode gastar no mês"
          value={money(metrics.safeToSpend, state)}
          tone={metrics.safeToSpend >= 0 ? "good" : "bad"}
        />
        <MetricCard
          label="Limite diário"
          value={money(metrics.dailyLimit, state)}
          tone={metrics.dailyLimit >= 0 ? "good" : "bad"}
        />
        <MetricCard
          label="Investimentos"
...
function BillsPage({
  ...
  const rows = state.bills
    .filter((b) => ym(b.dueDate) === month || !b.paid)
  ...
  const markPaid = (b: FutureBill) =>
    updateState((prev) => ({
      ...prev,
      bills: prev.bills.map((x) => (x.id === b.id ? { ...x, paid: true } : x)),
      transactions: [
        {
          id: uid("tr"),
            ...
            {rows.map((b) => (
              <tr key={b.id}>
                ...
                <td>
                  <MoneyInput
                    value={b.amount}
                    onChange={(value) => patch(b.id, { amount: value })}
                  />
                </td>
                <td>
                  <input
                    ...
                    checked={b.recurring}
                    onChange={(e) =>
                      patch(b.id, { recurring: e.target.checked })
                    }
                  />
                </td>
                <td>
                  <select
                    ...
                    onChange={(e) =>
                      patch(b.id, {
                        frequency: e.target.value as FutureBill["frequency"],
                      })
                    }
                  >
                ...
                <td>
                  <select
                    ...
                    onChange={(e) =>
                      patch(b.id, {
                        priority: e.target.value as FutureBill["priority"],
                      })
                    }
                  >
                    <option>Baixa</option>
                ...
                <td>
                  <input
                    type="checkbox"
                    checked={b.paid}
                    onChange={(e) => patch(b.id, { paid: e.target.checked })}
                  />
                </td>
